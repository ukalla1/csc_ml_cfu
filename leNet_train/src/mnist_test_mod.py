...
log_dir = "../bin/logs/fit/" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
tensorboard_callback = tf.keras.callbacks.TensorBoard(log_dir=log_dir, histogram_freq=1)

model.fit(x_train, y_train, epochs=32, batch_size=32, validation_data=(x_test, y_test), callbacks=[tensorboard_callback])
